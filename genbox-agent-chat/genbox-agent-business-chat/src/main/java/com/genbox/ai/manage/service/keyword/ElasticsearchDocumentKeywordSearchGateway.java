...
import com.genbox.ai.manage.model.DocumentRetrieveFilters;
import com.genbox.ai.manage.model.DocumentRetrieveRequest;
import com.genbox.ai.manage.model.es.DocumentKeywordIndexRecord;
import com.genbox.ai.manage.support.DocumentKnowledgeMetadataKeys;
...
public class ElasticsearchDocumentKeywordSearchGateway implements DocumentKeywordSearchGateway {
    ...
    public ElasticsearchDocumentKeywordSearchGateway(
        ...
        GenBoxAgentDocumentMapper documentMapper,
    ...
    @Override
    public void indexChunks(List<GenBoxAgentDocumentChunk> chunkList) {
        if (CollUtil.isEmpty(chunkList)) {
            return;
        }

        Map<Long, GenBoxAgentDocument> documentMap = loadDocumentMap(chunkList);
        BulkRequest.Builder bulkBuilder = new BulkRequest.Builder()
            .index(properties.getElasticsearch().getIndexName())
            .refresh(Refresh.WaitFor);

        for (GenBoxAgentDocumentChunk chunk : chunkList) {
            GenBoxAgentDocument document = documentMap.get(chunk.getDocumentId());
    ...
    private Map<Long, GenBoxAgentDocument> loadDocumentMap(List<GenBoxAgentDocumentChunk> chunkList) {
        List<Long> documentIds = chunkList.stream()
            .map(GenBoxAgentDocumentChunk::getDocumentId)
        ...
        List<GenBoxAgentDocument> documents = documentMapper.selectBatchIds(documentIds);
        Map<Long, GenBoxAgentDocument> documentMap = new LinkedHashMap<>();
    ...
    private DocumentKeywordIndexRecord toIndexRecord(GenBoxAgentDocumentChunk chunk, GenBoxAgentDocument document) {
