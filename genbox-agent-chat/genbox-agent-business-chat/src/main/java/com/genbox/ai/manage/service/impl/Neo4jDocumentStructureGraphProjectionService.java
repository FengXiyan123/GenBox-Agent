...
public class Neo4jDocumentStructureGraphProjectionService implements DocumentStructureGraphProjectionService {
    ...
    public void projectToGraph(Long documentId, Long parseTaskId) {
        ...
        List<GenBoxAgentDocumentStructureNode> nodes = documentStructureNodeService.listDocumentNodes(documentId, null);
        ...
        try (Session session = openSession()) {
            session.executeWrite(tx -> {
                ...
                for (GenBoxAgentDocumentStructureNode node : nodes) {
    ...
    private org.neo4j.driver.Value sectionParams(GenBoxAgentDocumentStructureNode node) {
    ...
    private org.neo4j.driver.Value itemParams(GenBoxAgentDocumentStructureNode node, DocumentStructureNodeTypeEnum nodeType) {
        return Values.parameters(
            "nodeId", node.getId(),
            "documentId", node.getDocumentId(),
            "parseTaskId", node.getParseTaskId(),
    ...
    private void mergeSectionSiblingEdges(org.neo4j.driver.TransactionContext tx, GenBoxAgentDocumentStructureNode node) {
    ...
    private void mergeItemSiblingEdges(org.neo4j.driver.TransactionContext tx, GenBoxAgentDocumentStructureNode node) {
        if (node.getNextSiblingNodeId() != null) {
            tx.run("""
    ...
    private void updateTaskGraphStatus(Long parseTaskId, String status, String errorMessage) {
        ...
        GenBoxAgentDocumentTask task = taskMapper.selectById(parseTaskId);
        if (task == null) {
            return;
        }
        try {
