...
import com.genbox.ai.chatagent.data.GenBoxAgentChatExchangeTraceStage;
import com.genbox.ai.chatagent.mapper.GenBoxAgentChatExchangeTraceStageMapper;
...
public class MybatisConversationTraceStageStore implements ConversationTraceStageStore {
    ...
    private final GenBoxAgentChatExchangeTraceStageMapper traceStageMapper;
    private final ObjectMapper objectMapper;

    @Resource
    private UidGenerator uidGenerator;

    public MybatisConversationTraceStageStore(GenBoxAgentChatExchangeTraceStageMapper traceStageMapper,
    ...
    public long startStage(String conversationId,
        ...
        GenBoxAgentChatExchangeTraceStage stage = new GenBoxAgentChatExchangeTraceStage();
        stage.setId(stageId);
    ...
    public void finishStage(long stageId,
        ...
        traceStageMapper.update(
            null,
            new LambdaUpdateWrapper<GenBoxAgentChatExchangeTraceStage>()
                .eq(GenBoxAgentChatExchangeTraceStage::getId, stageId)
                .set(GenBoxAgentChatExchangeTraceStage::getStageState, stageState.getCode())
                .set(GenBoxAgentChatExchangeTraceStage::getEndTime, new Date())
                .set(GenBoxAgentChatExchangeTraceStage::getDurationMs, Math.max(durationMs, 0L))
                .set(GenBoxAgentChatExchangeTraceStage::getSummaryText, StrUtil.blankToDefault(summaryText, ""))
                .set(GenBoxAgentChatExchangeTraceStage::getErrorMessage, StrUtil.blankToDefault(errorMessage, ""))
                .set(GenBoxAgentChatExchangeTraceStage::getSnapshotJson, writeNullableJson(snapshot))
        );
    }
    ...
    public List<ConversationTraceStageView> listStageViews(String conversationId, long exchangeId) {
        return traceStageMapper.selectList(
                new LambdaQueryWrapper<GenBoxAgentChatExchangeTraceStage>()
                    .eq(GenBoxAgentChatExchangeTraceStage::getConversationId, conversationId)
                    .eq(GenBoxAgentChatExchangeTraceStage::getExchangeId, exchangeId)
                    .orderByAsc(GenBoxAgentChatExchangeTraceStage::getStageOrder)
                    .orderByAsc(GenBoxAgentChatExchangeTraceStage::getStartTime)
                    .orderByAsc(GenBoxAgentChatExchangeTraceStage::getId)
            )
            .stream()
            .map(this::toView)
            .toList();
    ...
    private ConversationTraceStageView toView(GenBoxAgentChatExchangeTraceStage stage) {
