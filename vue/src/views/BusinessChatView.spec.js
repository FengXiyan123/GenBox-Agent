...
describe('BusinessChatView stream ownership', () => {
  it('keeps a visible manual theme switch in the chat toolbar', async () => {
    wrapper = mount(BusinessChatView, { attachTo: document.body })
    await flushPromises()

    const themeToggle = wrapper.get('[data-testid="theme-toggle"]')
    expect(themeToggle.attributes('aria-label')).toBe('切换为深色模式')

    await themeToggle.trigger('click')
    expect(themeToggle.attributes('aria-label')).toBe('切换为浅色模式')
