  ...
  <div :class="cn('admin-shell min-h-dvh bg-admin-bg', desktopCollapsed && 'admin-shell--collapsed')">
    ...
    <aside class="admin-sidebar hidden min-h-dvh border-r border-admin-border lg:flex">
      ...
      <nav class="flex min-h-0 flex-1 flex-col overflow-y-auto px-2 py-3" aria-label="后台主导航">
        <section v-for="group in navGroups" :key="group.label" class="admin-nav-group" :data-nav-group="group.tone">
          <h2 :class="cn('admin-nav-group__title mb-1 px-2 text-caption font-medium text-muted-foreground', desktopCollapsed && 'sr-only')">{{ group.label }}</h2>
      ...
      <DrawerContent class="h-dvh max-h-dvh rounded-none" data-testid="admin-mobile-drawer">
        ...
        <aside class="flex min-h-0 flex-1 flex-col">
          <div class="flex h-14 flex-none items-center gap-2 border-b border-border px-4">
            <img src="/GenBox_ico.png" alt="GenBox-Agent" class="size-8 flex-none rounded-md object-contain" />
            <strong data-brand-surface="mobile" class="text-body-sm font-semibold text-foreground">GenBox-Agent</strong>
          </div>
          <nav class="min-h-0 flex-1 overflow-y-auto px-3 py-4" aria-label="移动端后台导航">
            <section v-for="group in navGroups" :key="group.label" class="admin-nav-group" :data-nav-group="group.tone">
              <h2 class="admin-nav-group__title mb-1.5 px-2 text-caption font-medium text-muted-foreground">{{ group.label }}</h2>
              <div class="flex flex-col gap-1">
                <RouterLink
                  v-for="item in group.items"
                  :key="item.to"
...
.admin-nav-group {
  --nav-group-surface: var(--card);
  --nav-group-accent: var(--border-strong);
  margin-bottom: 0.75rem;
  padding: 0.375rem;
  border: 1px solid var(--border);
  border-left: 3px solid var(--nav-group-accent);
  border-radius: var(--radius-token-card);
  background: var(--nav-group-surface);
}
...
.admin-nav-group[data-nav-group='overview'] {
  --nav-group-surface: var(--nav-group-overview-surface);
  --nav-group-accent: var(--nav-group-overview-accent);
}

.admin-nav-group[data-nav-group='knowledge'] {
  --nav-group-surface: var(--nav-group-knowledge-surface);
  --nav-group-accent: var(--nav-group-knowledge-accent);
}

.admin-nav-group[data-nav-group='observability'] {
  --nav-group-surface: var(--nav-group-observability-surface);
  --nav-group-accent: var(--nav-group-observability-accent);
}
